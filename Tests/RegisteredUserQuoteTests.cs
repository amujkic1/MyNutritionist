...
{
	[TestClass]
	public class RegisteredUserQuoteTests
	{
		...
		public async Task Index_WritesQuote()
		{
			...
			{
				new Progress { PRId = 1, Date = DateTime.Now.AddDays(-6), ConsumedCalories = 1500, BurnedCalories = 200, RegisteredUser = registeredUserList[0] },
			});
			...
			var user = result.Model as RegisteredUser;
		...
		public async Task Index_WritesQuote_MultipleCalls()
		{
			...
			var user1 = result1.Model as RegisteredUser;
			var user2 = result2.Model as RegisteredUser;
			var user3 = result3.Model as RegisteredUser;
